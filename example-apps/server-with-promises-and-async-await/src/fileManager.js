...
function readStore() {
  return fs
    .readFile('./store.json', 'utf8') // read the file with a promise
    .then((contents) => JSON.parse(contents)) // when the file is read we parse it
    .catch((err) => {
      // if the file couldn't be read, we catch the error
      if (err.code === 'ENOENT') return [];
      // if the error is because the file doesn't exist we return an empty array
